...
// Unica fuente de verdad para el nombre visible de un alumno. No concatenar
// nombre + apellido a mano en ningun componente -- usar siempre esto.
//
// Por que hace falta el chequeo de "ya incluido": alumnos.nombre no
// garantiza ser solo el primer nombre. Las importaciones de Excel (y
// fichas viejas cargadas a mano) suelen traer el nombre completo en esa
// sola columna, con apellido null. Si despues esa ficha se vincula a una
// cuenta de alumno (register_alumno_profile) o se edita a mano
// (AlumnoPerfilSection), apellido puede terminar cargado por separado
// aunque el nombre ya lo incluya -- concatenar sin mas produce
// "Joaquin Tornay Tornay". Comparamos por sufijo normalizado (sin
// tildes/mayusculas) para que tambien cubra apellidos compuestos.
export function formatStudentName(alumno) {
  const nombre = (alumno?.nombre || '').trim()
  const apellido = (alumno?.apellido || '').trim()

  if (!apellido) return nombre || 'Alumno'
  if (!nombre) return apellido

  const nombreNormalizado = normalizeText(nombre)
  const apellidoNormalizado = normalizeText(apellido)

  const nombreYaTerminaEnApellido =
    nombreNormalizado === apellidoNormalizado ||
    nombreNormalizado.endsWith(` ${apellidoNormalizado}`)

  return nombreYaTerminaEnApellido ? nombre : `${nombre} ${apellido}`
}
...
export function getPreviousMonthKey(referenceDate = new Date()) {
