...
import { normalizeText } from './student-utils'
...
export function normalizarNombre(nombre) {
  const limpio = normalizeText(nombre)
...
export function clavePago(alumnoId, fecha_pago, monto, mes) {
  return `${alumnoId}|${mes || ''}|${fecha_pago}|${monto}`
}
...
export function construirResumenImportacion(filas, alumnos, pagos) {
  const existentes = new Set(
    (pagos || []).map((p) =>
      clavePago(
        ...
        p.fecha_pago,
        Number(p.monto),
        p.mes || mesDesdeFecha(p.fecha_pago)
      )
    )
  )
  ...
  const alumnosReutilizadosSet = new Set()
  ...
  filas.forEach((fila) => {
    ...
    if (alumnoId) {
      alumnosReutilizadosSet.add(alumno.nombre || fila.nombre)
    } else {
    ...
    const clave =
      alumnoId != null
        ? clavePago(alumnoId, fila.fecha_pago, fila.monto, fila.mes)
        : `nuevo|${fila.nombreClave}|${fila.mes}|${fila.fecha_pago}|${fila.monto}`
  ...
  return {
    ...
    alumnosReutilizados: Array.from(alumnosReutilizadosSet).sort(),
